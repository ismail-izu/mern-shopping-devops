...
export const loadUser = () => (dispatch: Function, getState: Function) => {
  ...
  axios
    .get('http://localhost:5001/api/auth/user', tokenConfig(getState))
    .then(res =>
      dispatch({
        type: USER_LOADED,
        payload: res.data
...
export const register = ({ name, email, password }: IAuthFunction) => (
  ...
  axios
    .post('http://localhost:5001/api/auth/register', body, config)
    .then(res =>
      dispatch({
...
export const login = ({ email, password }: IAuthFunction) => (
  ...
  axios
    .post('http://localhost:5001/api/auth/login', body, config)
    .then(res =>
      dispatch({
        type: LOGIN_SUCCESS,
        payload: res.data
